    ...
    async execute (client, commands){
        ...
        new Promise(async (resolve, reject) => {
            ...
            if (client.config.enablePlex) {
                ...
                .then(search => {
                    ...
                    if (search.status == 401) {
                        console.log(`[CONFIG] Plex configuration is invalid. Disabling Plex feature... Your Plex Authentication token is not valid.`)
                        client.config.enablePlex = false;
                    }

                    else if (search.status != 200) {
                        console.log(`[CONFIG] Plex configuration is invalid. Disabling Plex feature... Generic error.`)
                        client.config.enablePlex = false;
                    }
                })
                .catch(err => {
                    if (controller.signal.aborted) {
                        console.log(`[CONFIG] Plex configuration is invalid. Disabling Plex feature... Read more in the trace below:\n${controller.signal.reason}`)
                    }
            ...
            if (process.env.CFG_VERSION == null || process.env.CFG_VERSION != 1.3) {
                console.log(`[CONFIG] Your .ENV configuration file is outdated. This could mean that you may lose out on new functionality or new customisation options. Please check the latest config via https://github.com/ThatGuyJacobee/Elite-Music/blob/main/.env.example or the .env.example file as your bot version is ahead of your configuration version.`)
            }
            resolve();
        })
        .then(() => {
            console.log(`[CONFIG] Configuration loaded`)
            console.log("[STATUS] Loading successful. Core of the bot is ready!");
            console.log(`[STATUS] ${String(process.env.DEVMODE) == "true" ? "Dev version" : "Version"} ` + process.env.VERSION)
        })

        client.user.setActivity(client.config.presence, { type: 2 });

        client.on("voiceStateUpdate", (oldState, newState) => {
            if (newState.channel && newState.member.user.id === client.user.id) {
              const voiceChannel = newState.channel;
              const guild = voiceChannel.guild;
      
              console.log(`[VOICE] Bot joined voice channel "${voiceChannel.name}" in server "${guild.name}"`);
      
              const membersInChannel = voiceChannel.members.map(member => member.user.tag);
              console.log(`[VOICE] Members in the channel: ${membersInChannel.join(", ")}`);
            }
          });
    }
}