...
if (String(process.env.WEBSITE) == "true") {
    const express = require('express');
    const app = express();
    const port = 3000;
    
    app.get('/', (req, res) => {
        res.send(`Patrick the DJ Version: ${String(process.env.VERSION)}`);
    });
...
client.once('ready', async function() {
    console.log(`[CONFIG] Loading Configuration... (Config Version: ${process.env.CFG_VERSION || 'N/A'})`)
    const rest = new REST({ version: "10" }).setToken(process.env.TOKEN);

    try {
        await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
        console.log(`[CMDS] Commands registered (${String(process.env.DEVMODE) == "true" ? "DEV Mode" : "Production"})!`);
    } catch (err) {
        console.error(err);
    }
})
...
if (!process.env.TOKEN) {
    console.log(`[ERROR] The .env file could not be found/doesn't exist. Have you followed the setup instructions correctly (https://github.com/ThatGuyJacobee/Elite-Music) to ensure that you have configured your environment correctly?`)
    process.exit(0)
}

client.login(process.env.TOKEN)
.catch((err) => {
    console.log(`[ERROR] Bot could not login and authenticate with Discord. Have you populated your .env file with your bot token and copied it over correctly? (Using token: ${process.env.TOKEN})\nError Trace: ${err}`);
})
